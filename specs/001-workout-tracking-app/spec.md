...
- **FR-110**: O sistema DEVE lembrar o usuário periodicamente de realizar o backup, de forma discreta e não bloqueante, sem interromper a execução de um treino.
...
- **FR-122**: Quando a persistência não for concedida, o sistema DEVE sinalizar ao usuário, de forma compreensível e não bloqueante, que seus dados estão sujeitos a remoção pelo dispositivo, e DEVE aumentar a frequência do lembrete de backup de FR-110.
...
- **SC-028**: O aplicativo exibe a data do último backup e, quando o intervalo definido é ultrapassado, apresenta o lembrete sem interromper, bloquear ou exigir interação — verificado inclusive durante a execução de um treino.
...
- **SC-033**: Quando a persistência do armazenamento não é concedida, o usuário consegue identificar essa condição e o risco associado sem consultar ajuda, e o lembrete de backup passa a ocorrer com frequência maior — verificado em 100% dos casos de teste, inclusive durante a execução de um treino, onde não pode bloquear.
