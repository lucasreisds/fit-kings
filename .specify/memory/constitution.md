<!--
SYNC IMPACT REPORT — emenda de 2026-09-19
(material de apoio para revisão desta emenda; remover antes de commitar o arquivo emendado)

Mudança de versão: 1.3.0 → 1.4.0
Justificativa do incremento: MINOR. A regra de estilo visual da seção Restrições de Produto e
Plataforma teve sua orientação materialmente ampliada: sai uma preferência estética nomeada e
entram cinco restrições verificáveis. Nenhum princípio foi removido, renumerado ou redefinido de
forma incompatível, e nenhum piso do Princípio II foi flexibilizado.

Princípios modificados: nenhum. Os cinco permanecem como na v1.3.0.

Seção modificada:
  Restrições de Produto e Plataforma › Estilo visual — a obrigatoriedade de Glassmorphism é
     removida. Nenhum estilo específico passa a ser obrigatório: a direção visual é decisão da
     fase de implementação, delimitada por cinco restrições que prevalecem sobre qualquer
     preferência estética — contraste garantido por construção e fundos de texto estáveis;
     proibição de transparência, blur e efeitos dependentes do conteúdo sob o elemento na tela de
     execução; campos de carga, repetições e RIR como maior hierarquia visual; legibilidade sob
     luz forte e com brilho reduzido; efeitos que não comprometam rolagem nem a meta de 5 s por
     série. A exigência de uso da skill `frontend-design` permanece.

Por que é ampliação e não redefinição: a regra anterior já subordinava o estilo ao Princípio II.
Esta emenda substitui uma preferência por critérios verificáveis derivados do mesmo princípio, o
que aumenta o rigor em vez de relaxá-lo. Um estilo que antes seria admitido "com moderação" agora
precisa passar por cinco testes objetivos.

Seções inalteradas: Core Principles, Fluxo de Desenvolvimento e Portões de Qualidade, Governance.

Ratified inalterado: 2026-09-19. Last Amended permanece 2026-09-19 porque a emenda ocorre no
mesmo dia da ratificação; apenas a linha de versão muda.

TODOs diferidos: nenhum.

Origem desta emenda: decisão do proprietário de abrir a direção visual antes de `/speckit-implement`.
Artefatos ajustados na mesma ocasião: spec.md (Direcionamentos para a fase de planejamento),
plan.md, research.md (D9), tasks.md (T021, T115) e quickstart.md. Nenhuma inconsistência conhecida
permanece entre a constituição e a especificação.
-->
